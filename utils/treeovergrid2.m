function [E, varargout] = treeovergrid2( N, varargin )
...
%
% [E]=treeovergrid2(N) returns a tree edge set E over a grid with row-wise chains
% and random connectors. The tree E represents is undirected, i.e. if (i,j) is in E, then so is (j,i).
%
...
for ccnt=1:C
    ...
    connectorInd =  (ccnt-1)*M + stepfwd;
    ConnectorVs(ccnt) = V( connectorInd );
end

if C*M<N
    CV{end+1}=V([C*M+1:N]);
    connectorInd = C*M + stepfwd;
