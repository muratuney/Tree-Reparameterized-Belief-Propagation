function m = compmessageinterbp(this, E, l )
...
if isa( this.previnbox, 'gpot' ) && sum( this.previnboxlog(pptrs) ) == indeg
    
    % Find the indices of the parents on E except l 
    sumind = setdiff( [1:indeg], find( parents == l ) );
       
    Lambda_sum = zeros(dt, dt);
    nu_sum = zeros( dt, 1);
    for i=1:length( sumind )
        Lambda_sum = Lambda_sum + this.previnbox( pptrs(sumind(i)) ).Lambda;
        nu_sum = nu_sum + this.previnbox( pptrs(sumind(i)) ).nu;
    end
